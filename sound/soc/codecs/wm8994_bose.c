/*
 * wm8994_bose.c  --  WM8994 ALSA Soc Audio driver related Aries
 *
...
struct gain_info_t voipcall_gain_table[] = {
	{ /* SPK */
		...
		.reg  = WM8994_CLASSD,			/* 25h */
		.mask = WM8994_SPKOUTL_BOOST_MASK,
		.gain = 0x01 << WM8994_SPKOUTL_BOOST_SHIFT
	}, {
		.mode = VOIPCALL_SPK,
		.reg  = WM8994_CLASSD,			/* 25h */
		.mask = WM8994_SPKOUTR_BOOST_MASK,
		.gain = 0x01 << WM8994_SPKOUTR_BOOST_SHIFT
	}, {
		.mode = VOIPCALL_SPK,
		.reg  = WM8994_AIF1_DAC1_LEFT_VOLUME,	/* 402h */
		.mask = WM8994_AIF1DAC1L_VOL_MASK,
...
struct gain_info_t fmradio_gain_table[] = {
	{ /* HP */
		...
		.gain = WM8994_SPKOUT_VU | WM8994_SPKOUTL_MUTE_N | 0x33
	}, {
		.mode = FMRADIO_SPK,
		.reg  = WM8994_SPEAKER_VOLUME_RIGHT,	/* 27h */
		.mask = WM8994_SPKOUTR_MUTE_N_MASK | WM8994_SPKOUTR_VOL_MASK,
		.gain = WM8994_SPKOUT_VU | WM8994_SPKOUTR_MUTE_N | 0x33
	}, {
		.mode = FMRADIO_SPK,
		.reg  = WM8994_AIF1_DAC1_LEFT_VOLUME,	/* 402h */
		.mask = WM8994_AIF1DAC1L_VOL_MASK,
...
void wm8994_filter_camcording_sub(struct snd_soc_codec *codec)
{
	DEBUG_LOG("");

		wm8994_write(codec, WM8994_AIF1_DRC1_3, 0x0418);
		wm8994_write(codec, WM8994_AIF1_DRC1_1, 0x019B);
}

void wm8994_filter_voip_hp(struct snd_soc_codec *codec)
{
	DEBUG_LOG("");
	...
	/*AIF1DRC1*/
	wm8994_write(codec, 0x440, 0x10C);
	wm8994_write(codec, 0x441, 0x853);
	wm8994_write(codec, 0x442, 0x828);
	wm8994_write(codec, 0x443, 0x6BD);
	wm8994_write(codec, 0x444, 0x268);
}
...
void wm8994_record_main_mic(struct snd_soc_codec *codec)
{
	...
	val = wm8994_read(codec, WM8994_AIF1_CONTROL_1);
	val &= ~(WM8994_AIF1ADCL_SRC_MASK | WM8994_AIF1ADCR_SRC_MASK);
/*	val |= (WM8994_AIF1ADCR_SRC); */ /* For Stereo recording */
	wm8994_write(codec, WM8994_AIF1_CONTROL_1, val);
...
void wm8994_set_voicecall_common_setting(struct snd_soc_codec *codec)
{
	...
	wm8994_write(codec, WM8994_GPIO_11, 0xA101);
		...
		/*AIF2*/
	//	wm8994_write(codec,  0x211, 0x0033); /*AIF2_RATE 16KHz*/
	...
	//	wm8994_write(codec,  0x313, 0x0090); /*AIF2_BCLK 1024KHz*/
	//	wm8994_write(codec,  0x313, 0x00c0); /*AIF2_BCLK*/
		wm8994_write(codec,  0x313, 0x0070); /*AIF2_BCLK*/
	//	wm8994_write(codec,  0x314, 0x0020); /*AIF2_ADC_LRCLK*/
	//	wm8994_write(codec,  0x315, 0x0020); /*AIF2_DAC_LRCLK*/
	...
	else if(wm8994->clock_state == dap_connection_codec_slave){
		printk("[%s] codec_slave\n",__func__);
...
void wm8994_set_voicecall_receiver_audience(struct snd_soc_codec *codec)
{
	...
	audio_ctrl_mic_bias_gpio(wm8994->pdata, TWO_MIC_ON);
	...
	wm8994_write(codec, WM8994_POWER_MANAGEMENT_2,
		WM8994_TSHUT_ENA | WM8994_TSHUT_OPDIS | WM8994_MIXINL_ENA | WM8994_MIXINR_ENA |
		WM8994_IN1L_ENA | WM8994_IN1R_ENA);

	wm8994_write(codec, WM8994_INPUT_MIXER_2,
			WM8994_IN1LP_TO_IN1L | WM8994_IN1LN_TO_IN1L | WM8994_IN1RP_TO_IN1R | WM8994_IN1RN_TO_IN1R);
	...
	val = wm8994_read(codec, WM8994_RIGHT_LINE_INPUT_1_2_VOLUME);
	val &= ~(WM8994_IN1R_MUTE_MASK);
	wm8994_write(codec, WM8994_RIGHT_LINE_INPUT_1_2_VOLUME, val);  //hdlnc_ldj_0415_A1026
...
// add opk
	...
	if (wm8994->HAC_state)
		wm8994_set_codec_gain(codec, VOICECALL_MODE, VOICECALL_RCV_HAC);
	else
		wm8994_set_codec_gain(codec, VOICECALL_MODE, VOICECALL_RCV_AUDIENCE);
...
/*
	val = wm8994_read(codec, WM8994_POWER_MANAGEMENT_2);
	val &= ~(WM8994_TSHUT_ENA_MASK | WM8994_TSHUT_OPDIS_MASK |
		WM8994_MIXINL_ENA_MASK | WM8994_IN1L_ENA_MASK);
...
*/
	wm8994_write(codec, WM8994_INPUT_MIXER_2,
			WM8994_IN1LP_TO_IN1L | WM8994_IN1LN_TO_IN1L |
			WM8994_IN1RP_TO_IN1R | WM8994_IN1RN_TO_IN1R);
...
//	wm8994_write(codec, 0x03, 0x0300);
	...
	wm8994_set_codec_gain(codec, VOICECALL_MODE, VOICECALL_SPK_AUDIENCE);

#ifdef WM8994_FACTORY_LOOPBACK
	if (wm8994->loopback_path_control == LOOPBACK_SPK) {
		val = wm8994_read(codec, WM8994_LEFT_LINE_INPUT_1_2_VOLUME);
		val &= ~(WM8994_IN1L_VOL_MASK);
		val |= (WM8994_IN1L_VU | 0x09);
		wm8994_write(codec, WM8994_LEFT_LINE_INPUT_1_2_VOLUME, val);
		val = wm8994_read(codec, WM8994_RIGHT_LINE_INPUT_1_2_VOLUME);
		val &= ~(WM8994_IN1L_VOL_MASK);
		val |= (WM8994_IN1L_VU | 0x00);
		wm8994_write(codec, WM8994_RIGHT_LINE_INPUT_1_2_VOLUME, val);

		val = wm8994_read(codec, WM8994_SPEAKER_VOLUME_LEFT);
		val &= ~(WM8994_HPOUT1L_VOL_MASK);
		val |= (WM8994_HPOUT1_VU | 0x39);
		wm8994_write(codec, WM8994_SPEAKER_VOLUME_LEFT, val);
		val = wm8994_read(codec, WM8994_SPEAKER_VOLUME_RIGHT);
		...
		/* Using SUB mic on factory speaker loopback mode */
		wm8994_write(codec, 0x310, 0x8010);
		DEBUG_LOG("Exchange left right channel on factory mode");
	}
#endif
...
void wm8994_set_voicecall_headset(struct snd_soc_codec *codec)
{
	...
	if (wm8994->TTY_state)
		wm8994_set_codec_gain(codec, VOICECALL_MODE, VOICECALL_HP_TTY);
	else
...
int wm8994_set_codec_gain(struct snd_soc_codec *codec, u32 mode, u16 device)
{
	...
	if (mode == PLAYBACK_MODE) {
		...
		case VOICECALL_BT:
			...
			break;
		case VOICECALL_RCV_AUDIENCE:
			gain_set_bits |= VOICECALL_RCV_AUDIENCE;
			break;
		case VOICECALL_SPK_AUDIENCE:
			gain_set_bits |= VOICECALL_SPK_AUDIENCE;
			break;
		case VOICECALL_RCV_HAC:
			gain_set_bits |= VOICECALL_RCV_HAC;
			break;
		case VOICECALL_HP_TTY:
			gain_set_bits |= VOICECALL_HP_TTY;
			break;
		default:
		...
		case FMRADIO_SPK:
			...
			break;
		default:
			DEBUG_LOG_ERR("fmradio gain flag is wrong");
		}
		...
		case VOIPCALL_SUBMIC_OTHER:
			...
			break;
		default:
			DEBUG_LOG_ERR("voipcall gain flag is wrong");
			break;
		}
	}
	...
	if (mode == PLAYBACK_MODE &&
		...
		wm8994_filter_voip_other(codec);
			...
			(device == VOIPCALL_HP || device == VOIPCALL_HPMIC))
		wm8994_filter_voip_hp(codec);
	else if (mode == RECORDING_MODE &&
			(device == RECORDING_CAM_SUB))
		wm8994_filter_camcording_sub(codec);
	else
